                    <div class="menuTop">
                        <ul>
                            <li><a href="<?php echo base_url() ?>medico/solicitacoes">Página Principal</a><li>
                            <li><a href="<?php echo base_url() ?>medico/visualizaEditaMedicoMedicos">Editar Dados Pesoais</a><li>
                            <li><a href="<?php echo base_url() ?>agenda"> Cadastrar Agenda</a><li>
                            <li><a href="<?php echo base_url() ?>agenda/listarAgendasMedico"> Minhas Agendas</a><li>
                            <li><a href="<?php echo base_url() ?>clinica">Cadastrar Clínicas</a><li>
                           	<li><a href="<?php echo base_url() ?>clinica/listaClinicas">Editar Clínicas</a><li>
                        </ul>    
                    </div>