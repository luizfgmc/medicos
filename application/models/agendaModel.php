<?php
	...
	class AgendaModel extends CI_Model{
		...
		public function listaAgendasComSaldo(){
		...
		public function listaAgendasMedico(){
			...
			$this->db->select('a.medico_id, s.data_agendamento, a.data_emissao, a.id, a.quantidade, a.saldo, a.clinica_id, m.nome_medico, m.telefone, m.crm, e.descricao, c.nome, c.telefone, c.endereco, c.end_numero');
			$this->db->from('agendas as a');
			$this->db->join('solicitacoes as s', 's.agenda_id = a.id');
			$this->db->join('clinicas as c','a.clinica_id = c.id');
